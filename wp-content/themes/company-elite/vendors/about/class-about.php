...
if ( ! class_exists( 'Company_Elite_About' ) ) {
	...
	class Company_Elite_About {
		...
		/**
		 * Singleton instance of Company_Elite_About.
		 *
		 * @var Company_Elite_About $instance Company_Elite_About instance.
		 */
		private static $instance;

		/**
		 * Main Company_Elite_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Company_Elite_About ) ) {
				self::$instance = new Company_Elite_About;
		...
		public function setup_config() {
			...
			$dismiss_url = wp_nonce_url( add_query_arg( array( 'company-elite-at-dismiss' => '1' ), admin_url( 'themes.php' ) ), 'company-elite-action-dismiss-' . get_current_user_id(), 'company-elite-dismiss-nonce' );
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'company-elite' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'company-elite' ), $this->theme_name ) . '</a>' ) . '<a href="' . esc_url( $dismiss_url ) . '" class="btn-at-dismiss">' . esc_html__( 'Dismiss', 'company-elite' ) . '</a></p>';
		}
		...
		function hide_notice() {
			// Verify nonce.
			if (! ( isset( $_GET['company-elite-dismiss-nonce'] ) && wp_verify_nonce( sanitize_key( $_GET['company-elite-dismiss-nonce'] ), 'company-elite-action-dismiss-' . get_current_user_id() ) ) ) {
				return;
			}

			if ( 1 === absint( $_GET['company-elite-at-dismiss'] ) ) {
				update_user_meta( get_current_user_id(), 'company_elite_at_dismissed', 1 );
			}
		}
		...
		public function activation_admin_notice() {
			...
			$dismissed_status = get_user_meta( get_current_user_id(), 'company_elite_at_dismissed', true );
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'company-elite-about', get_template_directory_uri() . '/vendors/about/css/about' . $min . '.css', array(), '2.0.2' );
				wp_enqueue_script( 'company-elite-about', get_template_directory_uri() . '/vendors/about/js/about' . $min . '.js', array( 'jquery' ), '2.0.2' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'company-elite-about', 'companyEliteAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'company-elite' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'company-elite' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'company-elite' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'company-elite' );
		...
		public function useful_plugins() {
			...
			if ( isset( $tgmpa ) ) {
				...
				if ( true === $complete ) {
					echo '<div class="all-active"><span class="dashicons dashicons-yes"></span>&nbsp;' . esc_html__( 'Great, all recommended plugins are intalled and active.', 'company-elite' ) . '</div>';
				}
				...
				if ( true !== $complete ) {
					echo '<div class="button-wrap"><a href="' . esc_url( $url ) . '" class="button button-primary">' . esc_html__( 'Manage Plugins', 'company-elite' ) . '</a></div>';
				}
